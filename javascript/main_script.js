...
function send_websocket(message, sender, receiver, websocket){
    var json_message = {
...
function send_button(message_element, message_box_element, my_name, rcv_name, websocket) {
    var message = message_element.value;
    console.log(message);
    append_div_messages(my_name, message, message_box_element);
    send_websocket(message, my_name, rcv_name, websocket);
...
var rcv_name_element = document.getElementById("receiver_login_id");
...
button_element.onclick = () => {
    send_button(
        ...
        rcv_name_element.value,
        webSocket
    );
};

connect_button_element.onclick = () => {
    var url = `ws://${server_address_element.value}/`;
    console.log("url", url);
